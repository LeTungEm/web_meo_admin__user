<template>
    <div>
      <label class="text-gray-500" for="description"
        >Kinh nghiệm chăm sóc mèo</label
      >
      <froala
  ...
  export default {
    ...
    watch:{
      experienceData: function(){
        this.description = this.experienceData;
      }
    },
    props:{
    ...
    methods:{
      ...
      saveData(){
          SettingService.updateExperience(this.description).then(res => {
            console.log(res.data);
            this.showNotification(["Sửa thành công kinh nghiệm chăm sóc mèo", false]);
          })
      },
    },
  };
