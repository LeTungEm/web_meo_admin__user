...
export default {
  ...
  watch:{
    aboutData: function(){
      this.description = this.aboutData;
    }
  },
  props:{
    aboutData: String,
  ...
  methods:{
    ...
    saveData(){
        SettingService.updateAbout(this.description).then(res => {
          console.log(res.data);
          this.showNotification(["Sửa thành công about", false]);
        })
    },
  },
};
